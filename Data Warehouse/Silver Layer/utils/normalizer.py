...
class Normalizer:
    def __init__(self):
        pass
    def normalize_categorical_columns_manual_mapping(self,df, un_normalized_cols_name, normalized_cols_name, columns_map):
    ...
    def normalize_na_replacer_columns(self,df,nan_replacer_columns, cleaned_nan_replacer_columns,replacer_value="Not Available or Applicable"):
        for col,renamed_col in zip(nan_replacer_columns, cleaned_nan_replacer_columns):
            df[renamed_col] = df[col].fillna(replacer_value)
        return df


    def normalize_categorical_columns_non_exploding(self,df, multi_select_cols, multi_select_normalized,multi_select_maps):
    ...
    def normalize_categorical_columns_exploding(self,df, multi_select_cols, multi_select_normalized, multi_select_maps):
        bridge_results = {}
    ...
    def clean_years_columns(self, df, cols):
        for col in cols:
    ...
    def fill_na_and_remove_outlier_percentile_method(self, df, col, lower_percentile=0.01, upper_percentile=0.99):
